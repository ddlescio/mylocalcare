CREATE TABLE utenti (
        ...
        telefono TEXT,
        email_pubblica TEXT,
        indirizzo_studio TEXT,  -- sede o studio di riferimento
        sito_web TEXT,
        instagram TEXT,
        facebook TEXT,
        linkedin TEXT,
    ...
    , admin_browser_fingerprint TEXT, offro_9 INTEGER DEFAULT 0, offro_10 INTEGER DEFAULT 0, cerco_9 INTEGER DEFAULT 0, cerco_10 INTEGER DEFAULT 0, provincia TEXT, macro_area TEXT, maggiorenne_verificato INTEGER DEFAULT 0, data_verifica_maggiorenne TEXT, ip_verifica_maggiorenne TEXT, versione_consenso TEXT);
...
CREATE TABLE annunci (
        ...
        stato TEXT DEFAULT 'in_attesa', approvato_il TEXT, match_da_processare INTEGER DEFAULT 0, provincia TEXT, tipo_annuncio TEXT
CHECK (tipo_annuncio IN ('offro','cerco')), urgente INTEGER DEFAULT 0, ultima_notifica_urgente TEXT,
...
CREATE TABLE notifiche (
        ...
        scadenza_giorni INTEGER DEFAULT 10, titolo TEXT,
        FOREIGN KEY (id_utente) REFERENCES utenti(id)
    );
...
CREATE TABLE notifiche_admin (
    ...
    messaggio TEXT NOT NULL,
    link TEXT,
    tipo_invio TEXT NOT NULL,              -- notifica | email | entrambi
    filtro_json TEXT NOT NULL,              -- snapshot filtri usati
    tab_attivo TEXT NOT NULL,               -- u-zona, u-categoria, ecc
...
CREATE TABLE servizi (
    ...
    -- 'annuncio' | 'profilo' | 'chat' | 'homepage'

    target TEXT NOT NULL,
    -- 'offre' | 'cerca' | 'entrambi'

    durata_default_giorni INTEGER,
    -- es. 1, 3, 7, 30 (NULL se one-shot)

    ripetibile INTEGER DEFAULT 1,
    -- es. boost sì, verifica identità no

    attivabile_admin INTEGER DEFAULT 1,
    -- admin può attivarlo gratis/manualmente
...
CREATE TABLE acquisti_servizi (
    ...
    -- 'stripe' | 'paypal' | 'admin' | 'promo'

    importo REAL DEFAULT 0,
    valuta TEXT DEFAULT 'EUR',

    riferimento_esterno TEXT,
    -- id pagamento Stripe / PayPal
...
CREATE TABLE storico_servizi (
    ...
    -- 'creato' | 'scaduto' | 'revocato' | 'esteso'
...
CREATE INDEX idx_annunci_ultima_notifica_urgente
...
CREATE TABLE pacchetti_servizi (
  ...
  servizio_id INTEGER NOT NULL,
  created_at TEXT DEFAULT (datetime('now')), durata_override INTEGER,
...
CREATE TABLE acquisti (
  ...
  riferimento_esterno TEXT,
  created_at TEXT DEFAULT (datetime('now')), annuncio_id INTEGER,
  FOREIGN KEY (utente_id) REFERENCES utenti(id),
...
CREATE TABLE servizi_piani (
    ...
    durata_giorni INTEGER NULL,        -- ✅ ORA NULLABILE
...
CREATE INDEX idx_segnalazioni_messaggio ON segnalazioni_chat(messaggio_id);
CREATE TABLE video_call_log (
    ...
    created_at TEXT DEFAULT (datetime('now')), in_corso INTEGER DEFAULT 1, last_ping TEXT, ended_at TEXT,
    FOREIGN KEY (utente_1) REFERENCES utenti(id),
    FOREIGN KEY (utente_2) REFERENCES utenti(id)
);
CREATE INDEX idx_video_call_created ON video_call_log(created_at);
